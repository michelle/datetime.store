...
/** Shipping details for fulfilment from Stripe's purpose-built shipping field. */
...
export async function fulfillPaymentIntent(pi: Stripe.PaymentIntent): Promise<FulfillmentResult> {
  ...
  const response = await createOrder({
    ...
    await stripe()
      ...
      .catch(() => undefined);
    console.error(`[fulfillment] Prodigi rejected ${pi.id}: ${message}`);
    throw new FulfillmentError("The print order could not be placed yet", "prodigi_error", 502);
  });
