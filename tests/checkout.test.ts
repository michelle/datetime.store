...
test("timestamp validation and URL parsing", () => {
  const now = Date.now();
  assert.equal(isValidTimestamp(now), true);
  ...
  assert.equal(parseTimestampParam(`${now}.png`), now);
...
test("artwork canvas matches the Prodigi print area aspect ratio", () => {
  ...
  assert.equal(ARTWORK_PX.width, 4680);
  assert.equal(ARTWORK_PX.height, 5790);
});
