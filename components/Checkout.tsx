...
async function finalizeOrder(paymentIntentId: string, clientSecret: string): Promise<{ prodigiOrderId: string | null; error?: string }> {
  ...
  const pay = useCallback(
    async (shipping: ShippingDetails, payerEmail: string) => {
      ...
      try {
        ...
        const result = await finalizeOrder(paymentIntentId, clientSecret);
        onCompleted({ paymentIntentId, clientSecret, prodigiOrderId: result.prodigiOrderId, timestamp, email: payerEmail, fulfillmentError: result.error });
      } catch (err) {
