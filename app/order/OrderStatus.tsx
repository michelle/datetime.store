...
export default function OrderStatus() {
    ...
    (async () => {
      try {
        await fetch("/api/orders/finalize", {
          ...
          body: JSON.stringify({ paymentIntentId: id, clientSecret }),
        }).catch(() => undefined);
        const query = new URLSearchParams({ payment_intent_client_secret: clientSecret });
        const res = await fetch(`/api/orders/${encodeURIComponent(id)}?${query}`);
  ...
  if (!clientSecret) return <div className="alert alert-danger">This order link is incomplete. Use the full link from checkout.</div>;
